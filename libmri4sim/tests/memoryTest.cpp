...
TEST_GROUP_BASE(memoryTests, mri4simBase)
{
    void setup()
    {
        mri4simBase::setup();
...
{
    IMemory_Write8(m_pContext->pMemory, INITIAL_SP - 1, 0x5a);
    char command[64];
...
{
    IMemory_Write16(m_pContext->pMemory, INITIAL_SP - 2, 0xF00D);
    char command[64];
...
TEST(memoryTests, WriteByte)
{
    ...
    STRCMP_EQUAL(checksumExpected(), mockIComm_GetTransmittedData());
    CHECK_EQUAL(0x5A, IMemory_Read8(m_pContext->pMemory, INITIAL_SP - 1));
}
...
TEST(memoryTests, WriteHalfWord)
{
    ...
    appendExpectedString("+$OK#+");
    STRCMP_EQUAL(checksumExpected(), mockIComm_GetTransmittedData());
    CHECK_EQUAL(0xF00D, IMemory_Read16(m_pContext->pMemory, INITIAL_SP - 2));
}
